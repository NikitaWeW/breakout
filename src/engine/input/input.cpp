...
{
    engine::Registry &reg = *static_cast<engine::Registry *>(glfwGetWindowUserPointer(window));
...
void engine::input::setup(engine::Registry &reg)
{
...
void engine::input::update(engine::Registry &reg)
{
    ENGINE_ASSERT_MSG(!reg.view<engine::Window>().empty(), "forgot to add a window?");
    for(ecs::entity e_window : reg.view<engine::Window>())
    {
